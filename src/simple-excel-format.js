/**
 * Simple Excel formatting that preserves the exact data structure
 * while adding color formatting
 */
...
function formatEnhancedTimelineSheet(worksheet, range, appState) {
  ...
  for (let r = 2; r <= range.e.r; r++) {
    // Skip if this is a phase row
    const firstCellAddress = XLSX.utils.encode_cell({ r, c: 0 });
    const firstCell = worksheet[firstCellAddress];
    if (firstCell && firstCell.v && typeof firstCell.v === 'string' && firstCell.v.includes(':')) {
      continue;
    }
  ...
  // Format cost rows at the bottom (last 6 rows)
  const costRowLabels = [
    'Monthly Labor Cost',
